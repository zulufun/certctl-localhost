...
try:
    ...
    if ($cert) {
        ...
        # We need to configure chain policy to not do revocation checking just in case
        $chain.ChainPolicy.RevocationMode = 'NoCheck'
        $chain.Build($cert) | Out-Null
        Write-Output "Chain Elements Count: $($chain.ChainElements.Count)"
        foreach ($element in $chain.ChainElements) {
            Write-Output "Thumbprint: $($element.Certificate.Thumbprint)"
            $element.ChainElementStatus | ForEach-Object { Write-Output "  Status: $($_.Status)" }
        }
    }
    """
