...
export interface VerifyDeploymentRequest {
  /** Deployment target the agent probed */
  target_id: string;
  /** SHA-256 fingerprint of the certificate that should be served (hex, lowercase) */
  expected_fingerprint: string;
  /** SHA-256 fingerprint observed on the live TLS endpoint (hex, lowercase) */
  actual_fingerprint: string;
  /** True when expected and actual fingerprints match */
  verified: boolean;
  /**
   * Error message when probe failed or fingerprints differ
   * @nullable
   */
  error?: string | null;
}
