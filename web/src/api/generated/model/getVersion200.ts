...
export type GetVersion200 = {
  /** Release tag (ldflags-supplied) or VCS SHA fallback or "dev" */
  version: string;
  /** Git SHA from runtime/debug.BuildInfo (vcs.revision); empty when not VCS-tracked */
  commit: string;
  /** True when build had uncommitted changes (vcs.modified) */
  modified: boolean;
  /** RFC 3339 build timestamp (vcs.time); empty when not VCS-tracked */
  build_time: string;
  /** Go toolchain version that compiled the binary (runtime.Version()) */
  go_version: string;
};
